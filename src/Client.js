    ...
    async initialize(phone_number) {
        ...
        if (needAuthentication) {
            ...
            if (phone_number) {
                ...
                await page.waitForSelector(LINK_QRCODE_SELECTOR, { timeout: 0 });
                ...
                await Promise.race([
                    new Promise(resolve => {
                        page.waitForSelector(FONE_NUMBER_AUTHENTICATION_SELECTOR, { timeout: 0 })
                            .then(() => resolve(true))
                            .catch((err) => resolve(err));
                    })
                ]);
